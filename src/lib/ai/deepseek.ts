...
export const deepseekProvider: AIProvider = {
  tag: "deepseek:unconfigured",
  async generateReviewDraft(_input: ReviewInput): Promise<ReviewOutput> {
    throw new AIProviderError(
      "DeepSeek Provider 尚未接入（需先完成去标识化与二次评审），请切回 AI_PROVIDER=mock 或走教师手工撰写"
    );
  },
  async generateCorrectionAdvice(_input: CorrectionAdviceInput): Promise<CorrectionAdviceOutput> {
    throw new AIProviderError(
      "DeepSeek Provider 尚未接入（需先完成去标识化与二次评审），请切回 AI_PROVIDER=mock 或教师手工填写复核说明"
    );
  },
};
